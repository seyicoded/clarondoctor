        ...
        (async()=>{
            try{
                ...
                if(call_d != null){
                    ...
                    console.log('urgent cal data: ',urgent)
                    console.log('email: ', email)
                    navigation.navigate('Urgent', {channel: urgent.channel, token: urgent.token, email: email})
    ...
    useEffect(()=>{
        (async()=>{
            ...
            try{
                ...
                if(token == undefined){
                    token = await AsyncStorage.getItem('device_fcm_token');
                }
            }catch(e){
                token = await AsyncStorage.getItem('device_fcm_token');
            }
            ...
            try{
                await firestore().collection('device_token').doc(email).set({token: token});
            }catch(e){
                await firestore().collection('device_token').doc('error').set({token: e});
            }
        })()
    }, [])
