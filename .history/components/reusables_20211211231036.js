...
export const message = ({email, message, chat})=>{
    ...
    return (
        ...
        <Card onPress={()=>{
            console.log('clicked');
            try{
                if(chat.attachment == null || (chat.attachment).length < 3 ){
            ...
            {(chat.attachment != null && (chat.attachment).length > 3) && !(chat.file_type.includes('image')) ? <Image source={require('../assets/imga.png')} onError={()=>{console.log('e')}} style={{width: width-150, height: 180, marginBottom: 10}} resizeMode={'cover'}/> : null }
