...
export const message = ({email, message, chat})=>{
    ...
    let date = formatRelative(new Date(message.scheduledFor), new Date())
...
export const call = ({request, nav, respond}) => {
    // console.log(request)
    const patient_email = request.patient.email;
    const doctor_email = request.physician.email;
    // let date = new Date(request.scheduledFor).toString().substring(4, 21)
    let date = new Date(request.scheduledFor).toString().substring(4, 21)
    ...
    return (
        <Layout level={'2'} style={{marginVertical: 5, marginHorizontal: 10, borderRadius: 5, padding: 15}}>
            ...
            { !isBefore(new Date(request.scheduledFor), new Date()) ?
                <Divider/> : <></>
            }

            {/* { true ? request.status == 'Pending' ? */}
            { !isBefore(new Date(request.scheduledFor), new Date()) ? request.status == 'Pending' ?
            ...
            <Button appearance={'outline'} onPress={()=>nav.navigate('VideoCall', {doctor_email: doctor_email, patient_email: patient_email})} size={'small'}>Start Call</Button>
