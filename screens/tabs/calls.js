...
const Calls = ({navigation}) =>{
  ...
  const getSchedule = async ()=>{
    try{
      ...
      requests.forEach(request=>{
        !days.includes(new Date(request.scheduledFor).toString().substring(0, 10)) ? days.push(new Date(request.scheduledFor).toString().substring(0, 10)) : null
      })

      setselected(requests.filter(request=>{
        return new Date().toString().substring(0, 10) == new Date(request.scheduledFor).toString().substring(0, 10)
      }))
  ...
  const filter = (date)=>{
    setnow(date)
    setselected(requests.filter(request=>{
      return date.toString().substring(0, 10) == new Date(request.scheduledFor).toString().substring(0, 10)
    }))
  }
